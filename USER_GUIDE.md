...
一般情况下使用pigeon提供的random/roundRobin/autoawre这几种策略就足够了，如果需要自己实现负载均衡策略，可以在客户端的配置里添加loadBalanceClass属性，这个class必须实现com.dianping.pigeon.remoting.invoker.route.balance.LoadBalance接口，一般可以继承pigeon提供的AbstractLoadBalance抽象类或pigeon目前已有的loadbalance类。
...
该功能是对服务端处理请求时，由pigeon取消已经超时的客户端请求，pigeon在默认情况下会取消还未执行的超时请求，但如果已经执行的超时请求pigeon一般不会去取消（当服务端线程池全部打满的情况下会去取消已经执行的超时请求），如果需要任何情况下都取消已经执行的超时请求，需要进行以下设置：
在应用的classpath下放config/pigeon.properties文件，内容如下:
pigeon.timeout.cancel=true

注意：该设置只是设置线程的interupted状态，不能保证一定能取消已经执行请求，一般来说只能中断阻塞的线程，比如wait/sleep/join，对于正在运行的线程如果想响应这种状态，可以按以下方式写代码：
