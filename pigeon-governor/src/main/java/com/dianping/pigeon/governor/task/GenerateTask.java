...
public class GenerateTask implements Runnable {
    ...
    private void generateTasks(Environment env, String url, String group, 
    		String hosts, String pigeon2hosts) {
        if(hosts == null)
            return;
    ...
    class AddressRepo {
    	...
    	public void clear() {
    		for(Map.Entry<Environment, Set<String>> entry : addrRepo.entrySet()) {
    			...
    			logger.info(String.format("generated %d health check tasks for env %s[%s]", 
    					addrSet.size(), env, env.getZkAddress()));
    			addrSet.clear();
    		}
    	}
