...
public class FutureClient {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < 1; i++) {
			FutureClientThread thread = new FutureClientThread();
			thread.start();
		}
	}
...
class FutureClientThread extends Thread {
	...
	public void run() {
		...
		try {
			...
			for (;;) {
				...
				EchoService service = (EchoService) context.getBean("echoService_future");
				String input = "echoService_future-" + atomicInteger.incrementAndGet();
