...
public class InvocationUtils {
	...
	public static String getRemoteCallFullName(String serviceName,
		...
		if (parameterTypes != null) {
			...
			return getRemoteCallFullName(serviceName, methodName,
					parameterTypes_);
		} else {
	...
	public static String getRemoteCallFullName(String serviceName,
		...
		List<String> serviceFrags = Splitters.by("/").noEmptyItem()
				.split(serviceName);
		int fragLenght = serviceFrags.size();
		String name = "Unknown";
		if (fragLenght > 2) {
			StringBuilder sb = new StringBuilder(128);
			sb.append(serviceFrags.get(fragLenght - 2)).append(':')
					.append(serviceFrags.get(fragLenght - 1)).append(':')
					.append(methodName);
			sb.append('(');
	...
	public static Throwable toInvocationThrowable(Object responseReturn) {
		if (responseReturn == null) {
			...
			StackTraceElement[] stackTrace = (StackTraceElement[]) errors
					.get("stackTrace");
