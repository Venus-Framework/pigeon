...
public class ConnectExceptionInRPCInvokeListener extends AbstractRPCInvokeListener {
	...
	private static final Logger logger = LoggerLoader.getLogger(ConnectExceptionInRPCInvokeListener.class.getName());
	...
	public boolean support(RuntimeServiceEvent event) {
		if (event.getEventType() == RuntimeServiceEvent.Type.RUNTIME_RPC_INVOKE_CONNECT_EXCEPTION) {
			return true;
		}
		return false;
	}
